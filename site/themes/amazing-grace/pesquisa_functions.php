...
function mostra_entidades($do_link, $result) {
	...
	if($result != FALSE) {
		...
		foreach ($result as $row) {
			...
			if($do_link == 1)
				echo '<td><a target="_new" href="http://publicacoes.mj.pt/pt/Pesquisa.asp?sFirma=&amp;dfDistrito=&amp;dfConcelho=&amp;dInicial=&amp;dFinal=&amp;iTipo=0&amp;sCAPTCHA=&amp;pesquisar=Pesquisar&amp;dfConcelhoDesc=&amp;iNIPC='.$row['nif'].'">'.htmlspecialchars($row['ent']).'</a></td>';
			else
				echo '<td>'.htmlspecialchars($row['ent']).'</td>';
			$total+=$row['total'];
...
function mostra_ads($setas = 1, $search_val, $result) {
	...
	if($result != FALSE) {
                ...
                echo "<a href='/ad2csv?search_str=$search_val' title='Importe estes resultados para a sua folha de cálculo'><img src='".get_bloginfo('template_directory')."/images/Icon_csv.gif' border=\"0\" alt=\"CSV\" /></a> | ";
                echo "<a href='/ad2feed?search_str=$search_val' title='Subscreva esta pesquisa no seu leitor de notícias'><img src='".get_bloginfo('template_directory')."/images/rss.png' border=\"0\" alt=\"RSS\" /></a> | ";

		echo 'Foram encontrados <strong>'.count($result).'</strong> registos:';
		if ($setas == 1) {
			echo "<table class='search_results_table'><tr><th>ID</th><th class='data'><a href='/?search_str=$search_val&amp;sort=2'><img border='0' src='/wp-content/themes/amazing-grace/images/arrow_down.png' title='Mais recentes primeiro' alt='Mais recentes primeiro'/></a>&nbsp;Data&nbsp;<a href='/?search_str=$search_val&amp;sort=3'><img border='0' src='/wp-content/themes/amazing-grace/images/arrow_up.png' title='Mais antigos primeiro' alt='Mais antigos primeiro'/></a></th><th>Ent. Adjudicante</th><th>Ent. Adjudicada</th><th>Objecto</th><th><a href='/?search_str=$search_val&amp;sort=0'><img border='0' src='/wp-content/themes/amazing-grace/images/arrow_down.png' title='Ordem decrescente de montantes' alt='Ordem decrescente de montantes'/></a>&nbsp;Montante&nbsp;<a href='/?search_str=$search_val&amp;sort=1'><img border='0' src='/wp-content/themes/amazing-grace/images/arrow_up.png' title='Ordem crescente de montantes' alt='Ordem crescente de montantes'/></a></th></tr>";
		} else {
		...
		foreach ($result as $row) {
			...
			#echo '<td><a target="_new" href="http://publicacoes.mj.pt/pt/Pesquisa.asp?sFirma=&amp;dfDistrito=&amp;dfConcelho=&amp;dInicial=&amp;dFinal=&amp;iTipo=0&amp;sCAPTCHA=&amp;pesquisar=Pesquisar&amp;dfConcelhoDesc=&amp;iNIPC='.$row['nif_ent_adjudicante'].'">'.htmlspecialchars($row['ent_adjudicante']).'</a></td>';
			echo '<td>'.htmlspecialchars($row['ent_adjudicante']).'</td>';
			echo '<td><a target="_new" href="http://publicacoes.mj.pt/pt/Pesquisa.asp?sFirma=&amp;dfDistrito=&amp;dfConcelho=&amp;dInicial=&amp;dFinal=&amp;iTipo=0&amp;sCAPTCHA=&amp;pesquisar=Pesquisar&amp;dfConcelhoDesc=&amp;iNIPC='.$row['nif_ent_adjudicada'].'">'.htmlspecialchars($row['ent_adjudicada']).'</a></td>';
